...
| 3   | ![grounded update — the "Sources I Relied On" citation table](m4-citeData-1.png) ![critic passes: every claim traced to pulled data](m4-citeData-2.png) ![probe: get_activity withheld, Cortex escalates](m4-withheldActivity-1.png) ![critic passes the escalation — nothing invented](m4-withheldActivity-2.png) | a grounded update citing pulled activity, **and** the withheld-source case where Cortex refuses to invent the missing figures and escalates | M4   | ✅ **captured** — `m4-citeData-1/2.png` (grounded + cited) and `m4-withheldActivity-1/2.png` (withheld source → refusal); full traces `00-build/m4-happy-run.txt` / `m4-probe-run.txt` |
...
- **Screenshot #3 (M4, retrieve-vs-invent — two paired runs)** — the required deliverable is *both* halves of grounding, and because they are mutually exclusive by construction (one cites a figure, the other proves the figure can't be cited) they are two runs, not one.
