...
try:  # load .env if python-dotenv is installed; harmless if it isn't
    ...
    load_dotenv(override=True)  # override=True so .env wins over shell env (e.g. corporate proxy)
except ImportError:
    pass
...
PRICE_IN = float(os.environ.get("CORTEX_PRICE_IN_PER_M", "3.00"))
PRICE_OUT = float(os.environ.get("CORTEX_PRICE_OUT_PER_M", "15.00"))
...
def run(which: str = "happy") -> None:
    ...
    for step in range(1, MAX_ITERATIONS + 1):
        ...
        if tool_use_blocks:
            # Append the full assistant message (all content blocks)
        ...
        proposed = text_blocks[0].text if text_blocks else ""
